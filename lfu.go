...
func (q *BlocksQueue) down(position int) {
	...
	for {
		...
		if q.queue[parent].Priority < q.queue[smallestChild].Priority {
			break
		}
		q.swap(parent, smallestChild)
...
func main() {
	h := New()
	h.Push(&OrderedBlock{1, 1, 1, 1})
	h.Delete(1)
	//fmt.Println(h.Pop())
	h.Push(&OrderedBlock{5, 5, 5, 1})
	h.Push(&OrderedBlock{3, 3, 3, 1})
	h.Push(&OrderedBlock{2, 2, 2, 1})
	h.Push(&OrderedBlock{4, 4, 4, 1})
	h.Print()
	fmt.Println(h.Pop())
	fmt.Println(h.Pop())
	//h.Print()
	...
	//	h.Push(&OrderedBlock{r.Intn(999), r.Intn(999), i, i})
	//	i++
	//}
